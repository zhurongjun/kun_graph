...
template<typename T, typename TP = Less<>> KUN_INLINE void introSort(T begin, T end, TP&& p = TP())
{
    ...
    // setup stack
    Stack stack[32] = {{begin, end - 1, (Size)(std::log((float)(end - begin)) * 2.f)}};
    Stack current, inner;
    ...
    LOOP:
        ...
        Size count = current.max - current.min + 1;

        // out of depth, use heap sort
        if (current.max_depth == 0)
        {
            heapSort(current.min, count, std::forward<TP>(p));
            continue;
        }
        ...
        {
            // swap mid to head
            swap(*current.min, *(current.min + count / 2));
            ...
            while (true)
            {
                ...
                // swap bad point
                ::kun::swap(*inner.min, *inner.max);
            }
            // resume mid value
            ::kun::swap(*current.min, *inner.max);
